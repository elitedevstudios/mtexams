...
| Date | Milestone |
|---|---|
| 2026-07-14 | **Fixed iPad lesson videos not displaying** — embeds loaded from `youtube-nocookie.com` (separate domain from the `youtube.com` watch link), so under iPad Screen Time restrictions allowing youtube.com left the nocookie embeds blocked/blank. Switched embeds to `www.youtube.com/embed/` so one allow covers both. sw.js → v22 |
| 2026-07-14 | **Fixed blank Parent Dashboard print/PDF** — the certificate `@media print` rule in components.css (global) hid all body children on every page; scoped it with `:has(.certificate-overlay)` so the dashboard prints normally while certificate isolation still works. Verified selector logic both ways in-browser. sw.js → v21 |
